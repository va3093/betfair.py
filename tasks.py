...
@task
def generate_key(ctx, name=DEFAULT_NAME, bits=DEFAULT_BITS):
...
@task
def generate_cert(ctx, name=DEFAULT_NAME):
    key_file = '{0}.key'.format(name)
...
@task
def sign_cert(ctx, name=DEFAULT_NAME):
    key_file = '{0}.key'.format(name)
    csr_file = '{0}.csr'.format(name)
...
@task
def generate_pem(ctx, name=DEFAULT_NAME):
    key_file = '{0}.key'.format(name)
    crt_file = '{0}.crt'.format(name)
...
@task
def ssl(ctx, name=DEFAULT_NAME, bits=DEFAULT_BITS):
    generate_key(ctx,name, bits)
    generate_cert(ctx, name)
    sign_cert(ctx, name)
    generate_pem(ctx, name)
